...
const statusStyles: Record<Application['status'], { text: string; icon: string; color: string; bgColor: string }> = {
...
export const ApplicationCard: React.FC<ApplicationCardProps> = ({ 
    ...
    const { job, facilityLogo, status, appliedAt, $id, coverLetter, customMessage } = application;
    
    if (!job) return null;

    const statusInfo = statusStyles[status] || statusStyles['pending'];
    
    const applicationType = (coverLetter || customMessage) ? 'Personalizzata' : 'Rapida';
    
    const formattedDate = new Date(appliedAt).toLocaleDateString('it-IT', {
        day: 'numeric', 
        month: 'long', 
        year: 'numeric'
    });
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    ...
                    <div className="flex items-start space-x-4">
                        <img 
                            ...
                            className="w-14 h-14 rounded-lg object-cover border border-slate-100 shadow-sm flex-shrink-0"
                        />
                        <div>
                            <h3 
                                className="font-bold text-lg text-slate-800 hover:text-blue-600 cursor-pointer transition-colors line-clamp-1" 
            ...
            <div className="bg-slate-50 border-t border-slate-200 px-5 py-3 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 text-sm">
                ...
                <div className="flex items-center space-x-4 self-end sm:self-auto">
                    ...
                    <button 
                        onClick={() => onSelectJob(job.$id)} 
                        className="font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg transition-colors"
                    >
                        Vedi offerta
                    </button>
                </div>
