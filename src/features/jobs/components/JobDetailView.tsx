        ...
        <div className="bg-white rounded-2xl shadow-xl max-w-4xl mx-auto overflow-hidden relative pb-24 md:pb-0 border border-slate-100 animate-in fade-in duration-500">
            ...
            <div className="p-5 sm:p-8 border-b border-slate-100 bg-slate-50/50">
                <div className="flex justify-between items-start mb-6">
                    ...
                    >
                        <Icon type="arrow-left" className="w-5 h-5 mr-1 group-hover:-translate-x-1 transition-transform" />
                        Indietro
                    </button>
                    <div className="flex space-x-3">
                        <button className="p-2.5 rounded-full bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-blue-600 transition-colors shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-blue-200" title="Condividi">
                            <Icon type="share" className="w-5 h-5"/>
                        </button>
                        <button className="p-2.5 rounded-full bg-white border border-slate-200 text-slate-600 hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-colors shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-red-200" title="Salva offerta">
            ...
            <div className={`p-5 sm:p-8 border-t border-slate-100 bg-slate-50/50 grid grid-cols-1 ${((job as any).qualificationsRequired && (job as any).qualificationsRequired.length > 0) ? 'md:grid-cols-2 gap-8' : ''}`}>
                ...
                {(job as any).qualificationsRequired && (job as any).qualificationsRequired.length > 0 && (
                    <div>
                        <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
                        ...
                        <ul className="space-y-3">
                            {(job as any).qualificationsRequired.map((qual: string, i: number) => (
                                <li key={i} className="flex items-start text-slate-700 text-[15px]">
